...
const schema = z.object({
  ...
  razorpayKeyId: z.string({ message: 'RAZORPAY_KEY_ID is required' }).min(1),
  razorpayKeySecret: z.string({ message: 'RAZORPAY_KEY_SECRET is required' }).min(1),
  razorpayWebhookSecret: z.string({ message: 'RAZORPAY_WEBHOOK_SECRET is required' }).min(1),
  msg91AuthKey: z.string({ message: 'MSG91_AUTH_KEY is required' }).min(1),
  msg91SenderId: z.string({ message: 'MSG91_SENDER_ID is required' }).min(1),
  msg91TemplateOtp: z.string().min(1).default('MISHRN_OTP'),
